...
import { tuman, star, unstar } from "../../assets/media";

function CategoriesProduct({img, title, price, pplRated, rate}) {
    return <ColumnFlexCon className={'cp'}>
        <div>
            <div className="cp_image_container">
                <img src={img} alt="it's just a ring" className="cp_image" />
                ...
                <div className="blur br_20 no_vis transition"></div>
            </div>
            <h2 className="cp_header">{title}</h2>
        </div>
        <ColumnFlexCon className={'full_width'}>
            <RowFlexCon className={'flex_start full_width'}>
                <img alt='tuman_logo' src={tuman} className="tuman_logo" />
                <h2 className="favourite_info">{price}</h2>
            </RowFlexCon>
            <RowFlexCon className={'full_width'}>
                <RowFlexCon className={'flex_start'}>
                    <p className="favourite_detail">({pplRated})</p>
                    {rate.map((c, i) => (
                        <img alt={'star'} src={c ? star : unstar} className='star' key={i} />
                    ))}
                </RowFlexCon>
                <h3 className="color_green">موجود</h3>
            </RowFlexCon>
        </ColumnFlexCon>
    </ColumnFlexCon>
}
